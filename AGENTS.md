...
A Discord bot that runs a user-supplied Lua script to choose ping recipients, previews the result,
...
3. Add a `language` option to `/ping` in `src/discord/register.ts` and pass the chosen id through
   `preview(...)` in `src/discord/ping.ts`. The editor modal already carries the id in its
