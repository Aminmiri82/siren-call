...
try {
  ...
  engine.global.set('resolve_member', (reference: string) => {
    ...
    try { values.push(resolveMember(reference, context), undefined); }
    catch (error) { values.push(undefined, error instanceof Error ? error.message : 'Member lookup failed.'); }
    return values;
  });
  ...
  if (/not enough memory|out of memory/i.test(raw)) message = 'This script uses too much memory. Try a smaller selection or a simpler loop.';
